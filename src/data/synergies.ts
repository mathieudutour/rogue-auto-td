...
export const SYNERGIES: SynergyData[] = [
  ...
  },
  {
    ...
    tiers: [
      { count: 2, description: '20% crit chance (1.8x dmg)', bonuses: { critChance: 0.2, critMult: 1.8 } },
      { count: 4, description: '30% crit (2.2x), +30% dmg', bonuses: { critChance: 0.3, critMult: 2.2, damageMult: 1.3 } },
      {
        count: 6,
        description: 'REAPER: 40% crit (2.5x), +50% dmg, execute below 15% HP',
        bonuses: { critChance: 0.4, critMult: 2.5, damageMult: 1.5, executeThreshold: 0.15 },
      },
    ],
  },
  {
    ...
    tiers: [
      ...
      { count: 4, description: '+20% dmg, +18% range, +10% AS to ALL allies', bonuses: { damageMult: 1.2, rangeMult: 1.18, attackSpeedMult: 1.1, allAllies: true } },
      {
        count: 6,
  ...
  },
  {
    ...
    tiers: [
      ...
      {
        count: 6,
        description: 'OVERCHARGE: +55% AS, chain to 3 (75% dmg, 90px), +30% dmg',
        bonuses: { attackSpeedMult: 1.55, chainOnHit: 3, chainDamageFrac: 0.75, chainRange: 90, damageMult: 1.3 },
      },
    ],
  },
  {
    ...
    tiers: [
      { count: 2, description: '+30% damage, execute below 10% HP', bonuses: { damageMult: 1.3, executeThreshold: 0.1 } },
      {
        count: 4,
        description: 'OBLIVION: +55% dmg, execute below 18% HP, +1 gold/kill',
        bonuses: { damageMult: 1.55, executeThreshold: 0.18, bonusGoldOnKill: 1 },
      },
    ],
  },
  ...
  },
  {
    ...
    tiers: [
      ...
      { count: 4, description: '+55% AS, +25% dmg', bonuses: { attackSpeedMult: 1.55, damageMult: 1.25 } },
      {
        count: 6,
        description: 'LETHALITY: +85% AS, +45% dmg',
        bonuses: { attackSpeedMult: 1.85, damageMult: 1.45 },
      },
    ],
  },
  {
    ...
    tiers: [
      { count: 2, description: '+30% range, +20% dmg to guardians', bonuses: { rangeMult: 1.3, damageMult: 1.2 } },
      { count: 4, description: '+40% range, +35% dmg, +20% AS to guardians', bonuses: { rangeMult: 1.4, damageMult: 1.35, attackSpeedMult: 1.2 } },
      {
        count: 6,
        description: 'BASTION: +20% range, +20% dmg, +15% AS, +1 gold/kill to ALL allies',
        bonuses: { rangeMult: 1.2, damageMult: 1.2, attackSpeedMult: 1.15, bonusGoldOnKill: 1, allAllies: true },
      },
    ],
  },
];
